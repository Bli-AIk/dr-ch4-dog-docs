...
欢迎回来！这是《从零做一个 Deltarune 同人游戏》系列的第 9 集，也是**第一章的第六集**。

第 06、07 集做了第一回合的骨头弹幕和第二回合的 GB 演出，第 08 集把 ACT 与饶恕流程补完——三个回合只剩最后一个没做。这一集：**第三回合，车**。
...
前两个回合的弹幕是"剧本式"的：骨头按固定路径横穿，GB 有完整的蓄力演出，只有选目标是随机的（TARGET_WEIGHTS 权重轮盘）。第三回合，我们想加第三种感觉：**随机**。
...
顺带对比第二回合的 TARGET_WEIGHTS：那是**权重随机**，管"打谁"（苏西权重低，是为了给她留说话机会）；车回合的落点随机管"怎么躲"。两种随机各管一摊，别混着用。

另外，这一回合几乎不需要新文本——没有对话、没有旁白，唯一的文本是回合提示（战斗框下方的提示行）。第三回合轮到这一条：
...
"正在摇尾巴"——撞人之前的预告。这三条存在 dog.lua 的 `self.text` 数组里，引擎每回合从中挑一条显示在屏幕底部：

```lua
    -- Text randomly displayed at the bottom of the screen each turn
    -- 屏幕底部的回合提示文本，引擎每回合随机挑一条
    -- （键值对见上面的 JSON：前三回合依次轮到 turn_1/2/3）
    self.text = {
