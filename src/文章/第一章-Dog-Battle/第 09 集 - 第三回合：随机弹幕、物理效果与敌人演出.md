...
1. **有序滚动**：30 次撞击，骨头落点从左到右扫 3 个来回——整体覆盖稳定，玩家有规律可循；
...
"正在摇尾巴"——撞人之前的预告。这三条存在 dog.lua 的 `self.text` 数组里。注意：**默认是随机抽取**——引擎每回合从 `self.text` 里随机挑一条显示在屏幕底部，我们没有指定固定旁白，所以"第三回合必定摇尾巴"这种保证是不存在的：
...
列表只是注册，回合到底用哪个 wave，由 `selectWave` 决定——Dog 覆写了基类，前三个回合按回合数固定顺序：
...
回合选择就绪。接下来是 `scripts/battle/waves/car.lua`——整个文件比前两个回合大，因为演出是"编"出来的。它的常数（时长、距离、次数、随机范围）全部定义在文件头部，我们**讲到哪个函数，就解释它用的那些常数**，别一上来就背。先看 init——回合的状态都在这：
...
每根骨头 66 伤害，碰到就是重伤。这也是为什么前面的随机设计要"能读"：玩家得看出骨头往哪飞，才有得躲。

## 加入物理效果

撞击要"有重量"，三个引擎自带的能力就够了：
...
**爆炸贴图**——粒子是"碎屑"，再来一张贴图当"闪光"。`spawnCarBoom` 把 car_boom 贴图放在最顶层（`EFFECT_LAYER`），2 帧后移除：
...
`getCarFrontOffset` 用 car 动画贴图的偏移，算出"车头"到 Dog 锚点的距离——这样冲撞目标是"车头撞上战斗框边缘"，而不是锚点陷进去。
...
震动方向也是随机的（±4 的两个方向独立抽）——这就是"随机只影响小处"：骨架（撞 30 次）不变，每次的震感方向不一样。
...
`restoreDog` 把位置、缩放、动画全部还原（`onEnd` 也会调用它兜底）：
...
对照第 03 集清单，第 5 行的"车"部分——打勾。三个回合至此全部完成：骨头、GB、车。
