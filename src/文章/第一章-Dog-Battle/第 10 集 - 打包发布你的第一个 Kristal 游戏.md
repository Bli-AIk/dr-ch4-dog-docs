...
打包不是"把文件夹压个zip然后发群里就完事儿了"。

你得**决定什么进包**。判断标准只有一条：**普通玩家玩的时候（运行时）需要吗？**
...
另外，构建脚本目前只出 Windows x64 的包。安卓构建虽然有，但目前还是实验性的——如果你真的非常需要安卓构建，可以来 Kristal 中文交流群（群号：1062361876）交流。

什么时候给哪种？mod zip 适合圈内分享和快速迭代；独立包适合正式发布——门槛最低，一个 exe 解决一切。

## 自定义图标

游戏的第一印象是图标——窗口图标、主菜单列表图标、exe 文件图标。我们的项目目前还没自定义图标，跑起来用的是引擎默认的。给它换上自己的，分三步，从易到难：

**第一步，窗口图标**——在 mod 根目录放一张 `window_icon.png`。引擎加载 mod 时会自动读它：运行这个 mod 时，窗口标题变成 mod 的名字，窗口图标变成这张图（mod.json 的 `setWindowTitleAndIcon` 默认就是开启的，不用配置）。**独立包也一样生效**——独立包运行时加载的还是你的 mod，窗口图标同样来自这张图。

**第二步，主菜单列表图标**——在 mod 根目录放一张 `icon.png`，主菜单的 mod 列表里就会显示它。

**第三步，exe 文件图标（进阶）**——资源管理器里看到的 exe 图标来自引擎的 `icon.ico`，换它需要重编译 `icon.res`（用 Windows SDK 的 `rc` 命令，或者参考 LÖVE 官方的分发指南）。这步要碰 Windows 工具链，真有需要再研究——官方 wiki 的 [Releasing Projects](https://kristal.cc/wiki/releasing-mods) 页有完整步骤。

图片用 PNG 就行，尺寸随意——窗口图标通常用正方形（32×32 或 64×64 都行），引擎会自动缩放。素材按 EX02 的规矩来：自己画，或者用合规素材。

## 测试发布包

发布前最后一分钟检查：**用玩家视角跑一遍**。别用你自己的开发环境——用打好的独立包，或者把打好的 mod 包装进一个干净的 Kristal：

1. 把 `dr-ch4-dog-mod.zip` 放进干净 Kristal 的 `mods/` 文件夹；
2. 启动，mod选择里面有出现 dr-ch4-dog；
3. 进战斗：三回合弹幕、ACT、饶恕胜利；
4. 故意死一下，确认失败流程正常（全灭 → 游戏结束）；
5. 切一下语言——发布包里 en 和 zh_hans 都在，切换要正常（kristal-i18n 默认跟随系统语言）；
6. 退出，确认存档和设置没被污染。

独立包额外检查：窗口标题和图标是自己的；``Shift+` `` 的 debug 菜单也按不出来（RELEASE_MODE 生效）；存档目录独立（identity 改了，不会和引擎本体共用存档）。

电脑上多放一个干净的 Kristal 不费事，但"发出去才发现跑不了"就尴尬了。
...
包打好了，最后一步：**放到别人能下载的地方**。你可能还不知道：**GitHub 能自动打包**。
